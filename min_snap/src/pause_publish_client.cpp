...
int main(int argc, char ** argv)
{
  ...
  auto node = rclcpp::Node::make_shared("min_snap_pause_publish_client");
  const std::string service_name = argc > 1 ? argv[1] : "/min_snap/pause_trajectory_publish";
  ...
  if (response->success) {
    RCLCPP_INFO(node->get_logger(), "Pause request accepted: %s", response->message.c_str());
  } else {
    RCLCPP_ERROR(node->get_logger(), "Pause request rejected: %s", response->message.c_str());
  }
